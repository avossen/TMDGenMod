...
namespace TMDGen {
   namespace DF {
      ...
      f1Tperp_BCR08_t::f1Tperp_BCR08_t() : BCR08_t() {
         constr_msg = "f1Tperp: using BCR08";
      };

      double f1Tperp_BCR08_t::func_s( const Var_t& var ) const {
      ...
      double f1Tperp_BCR08_t::func_a( const Var_t& var ) const{
         double pT2 ( var.pT * var.pT );
         double M ( var.x*PROTON_MASS + m );
         double one_minus_x ( 1. - var.x );
         ...
         double output( var.x * M * PROTON_MASS / L_sq / denom );
         output *= temp;
         output *= C_F_alpha_S;
         output *= 0.5;
         output *= ONE_OVER_EIGHT_PI_CUBED;
      ...
      double f1Tperp_BCR08_t::func_aprime( const Var_t& var ) const{
         double pT2 ( var.pT * var.pT );
         double M ( var.x*PROTON_MASS + m );
         double one_minus_x ( 1. - var.x );
